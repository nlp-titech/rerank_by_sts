...
class W2V_REP_RERANKER:
    ...
    def rerank(self, queries, docs):
        ...
        for i, (qid, query) in tqdm(enumerate(queries.items()), total=len(queries)):
            ...
            q_rep = self.q_rep_pooler(q_embed, t_query)
...
class DENSE_RERANKER(W2V_REP_RERANKER):
    def score_func(self, q_rep, d_rep, t_query_id, t_doc_id, qid, drank):
...
class DENSE_AVE_RERANKER(DENSE_RERANKER):
    def q_rep_pooler(self, q_embed, t_query_id):
        if self.use_idf:
            ...
            q_rep = np.average(q_embed, axis=0, weights=q_weight)
        else:
            q_rep = np.average(q_embed, axis=0)
    ...
    def d_rep_pooler(self, d_embed, t_doc_id):
        if self.use_idf:
            ...
            try:
                d_rep = np.average(d_embed, axis=0, weights=d_weight)
            except ValueError:
                print(d_weight.shape, d_embed.shape)
                d_rep = np.average(d_embed, axis=0)
        else:
            d_rep = np.average(d_embed, axis=0)
...
class DENSE_MAX_RERANKER(DENSE_RERANKER):
    def q_rep_pooler(self, q_embed, t_query_id):
        if self.use_idf:
            q_weight = np.array([self.idf[t] for t in t_query_id])
            q_rep = np.max(q_embed * q_weight[:, np.newaxis], axis=0)
        else:
            q_rep = np.max(q_embed, axis=0)
        q_rep /= np.linalg.norm(q_rep)
        return q_rep
    ...
    def d_rep_pooler(self, d_embed, t_doc_id):
        if self.use_idf:
            d_weight = np.array([self.idf[t] for t in t_doc_id])
            try:
                d_rep = np.max(d_embed * d_weight[:, np.newaxis], axis=0)
            except ValueError:
                print(d_weight.shape, d_embed.shape)
                d_rep = np.max(d_embed, axis=0)
        else:
            d_rep = np.max(d_embed, axis=0)
        d_rep /= np.linalg.norm(d_rep)
        return d_rep
    ...
    def q_rep_pooler(self, q_embed, t_query_id):
        if self.use_idf:
            ...
            try:
                ...
                print(q_weight.shape, q_embed.shape)
                print(t_query_id)
                raise ValueError()
        else:
            q_rep = np.average(q_embed, axis=0)
...
class MAX_COEF_GLOBAL_RERANKER(COEF_GLOBAL_RERANKER):
    ...
    def d_rep_pooler(self, d_embed, t_doc_id):
        if self.use_idf:
            d_weight = np.array([self.idf[t] for t in t_doc_id])
            try:
                d_rep = np.max(d_embed * d_weight[:, np.newaxis], axis=00)
            except ValueError:
                print(d_weight.shape, d_embed.shape)
                d_rep = np.max(d_embed, axis=0)
        else:
...
class COEF_POOL_RERANKER(W2V_REP_RERANKER):
    def __init__(
        self,
        idf,
        use_idf,
...
class TOKEN_COEF_POOL_RERANKER(COEF_POOL_RERANKER):
    ...
    def q_rep_pooler(self, q_embed, t_queru):
        return q_embed

    def d_rep_pooler(self, d_embed, t_doc):
        return d_embed
...
class LOCAL_SOFT_BM25_RERANKER(W2V_REP_RERANKER):
    def __init__(
        self,
        idf,
        use_idf,
...
class AVE_SOFT_BM25_RERANKER(GLOBAL_SOFT_BM25_RERANKER):
    def q_rep_pooler(self, q_embed, t_query):
        if self.use_idf:
            q_weight = np.array([self.idf[t] for t in t_query])
            q_rep = np.average(q_embed, axis=0, weights=q_weight)
        else:
            q_rep = np.average(q_embed, axis=0)
        q_rep /= np.linalg.norm(q_rep)
    ...
    def d_rep_pooler(self, d_embed, t_doc):
        if self.use_idf:
            d_weight = np.array([self.idf[t] for t in t_doc])
            try:
                d_rep = np.average(d_embed, axis=0, weights=d_weight)
            except ValueError:
                print(d_weight.shape, d_embed.shape)
...
class MAX_SOFT_TF(W2V_REP_RERANKER):
    def score_func(self, q_rep, d_rep, t_query, t_doc, qid, drank):
...
class T2T_RERANKER(W2V_REP_RERANKER):
    def score_func(self, q_rep, d_rep, t_query, t_doc, qid, drank):
        sim_mat = np.dot(q_rep, d_rep.T)
        if self.use_idf:
            weight = np.array([self.idf[t] for t in t_doc])
