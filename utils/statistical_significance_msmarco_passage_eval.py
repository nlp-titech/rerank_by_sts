...
Modify for statistical test by Hiroki Iida <hiroki.iida@c.nlp.titech.ac.jp> at 18/6/2021
"""
import re
import sys
import scipy.stats
...
def main():
    ...
    if len(sys.argv) == 4:
        ...
        first_scores = compute_metrics_from_files(path_to_reference, path_to_candidate1)
        second_scores = compute_metrics_from_files(path_to_reference, path_to_candidate2)
        print(
            round(np.mean(first_scores), 4),
            round(np.std(first_scores, ddof=1), 4),
